...
for (let i=0;i<s.length;i++){
  ...
  if (c === '{' || c === '(' || c === '[') stack.push({ch:c,i, line: s.slice(0,i).split('\n').length});
  if (c === '}' || c === ')' || c === ']'){
    const o = stack.pop();
    if (!o){ console.log('Unmatched closing', c, 'at', i, 'line', s.slice(0,i).split('\n').length); process.exit(0); }
    const match = o.ch==='{'?'}':(o.ch==='(')?')':']';
    if (match !== c){ console.log('Mismatched', o.ch, 'with', c, 'at', i, 'line', s.slice(0,i).split('\n').length); process.exit(0); }
  }
}
